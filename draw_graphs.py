...
class ExperimentDrawer(ExperimentHandler):
    ...
    def saving_results_from_iperf(self, experiment) -> None:
        ...
        for columns, out_file, title in zip(needed_columns, out_files, [f"{experiment}_iperf_cwnd", f"{experiment}_iperf_speed"]):
            data = genfromtxt(input_filename, delimiter=',', skip_header=1, names = ("time", "bitrate", "cwnd"), usecols=tuple(columns))
            print(data)
            plt.plot(data)
    ...
    def saving_results_from_dmesg(self, experiment) -> None:
        ...
        for columns, out_file, title in zip(needed_columns, out_files, [f"{experiment}_dmesg_cwnd", f"{experiment}_dmesg_speed"]):
            data = genfromtxt(input_filename, delimiter=',', skip_header=1, names = ("time", "CWND", "estimated_speed"), usecols=tuple(columns))
            plt.xlabel(columns[0])
            plt.ylabel(columns[1])
            plt.xlim(0,60)
            plt.ylim(0,2000)
